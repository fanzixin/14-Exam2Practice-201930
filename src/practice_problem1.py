...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        # ---------------------------------------------------------------------
        new_contents = self.contents + additional_contents

        if self.volume < len(new_contents):
            self.contents = new_contents[0:self.volume]
            return new_contents[self.volume:len(new_contents)]
        else:
    ...
    def shrink(self, new_volume):
        ...
        if self.volume < len(self.contents):
            self.contents = contents[0:self.volume]
            return contents[self.volume:len(contents)]
        else:
            return ''
